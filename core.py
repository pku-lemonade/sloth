...
from config import CoreConfig, ScratchpadConfig
from noc import Link, Router
from sim_type import Task, Instruction, Read, Write, Conv, Pool, FC, Send
...
class Scheduler:
    def __init__(self, program, spm):
        self.program = sorted(program, key=lambda x:x.layer_id)
        self.spm = spm
        self.graph = None
...
class Core:
    def __init__(self, env, config: CoreConfig, program: List[List[Instruction]], id: int):
        ...
        self.scheduler = Scheduler(self.program, self.spm_manager)
